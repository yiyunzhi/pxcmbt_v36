...
class FlatDict(MutableMapping):
    ...
    def get_key_start_with(self, s):
        """
        Method to get key starts with given string

        :param s: str
        :return: list
        """
        return list(filter(lambda x: x.startswith(s), self.keys())) if s else self.keys()

    def get_value_by_key_start_with(self, s):
        """
        Method to get value whose keys starts with given string

        :param s: str
        :return: list
        """
        _l = self.get_key_start_with(s)
        return [self.__getitem__(x) for x in _l]

    def get_key_value_by_key_start_with(self, s):
        """
        Method to get key and value whose keys starts with given string

        :param s: str
        :return: list,list
        """
        _l = self.get_key_start_with(s)
        return _l, [self.__getitem__(x) for x in _l]
