...
from .class_element_userdata import SimpleStateElementUserdata
from .define import *
...
class SimpleStateElement(DiagramRoundRectElement):
    ...
    def __init__(self, **kwargs):
        ...
        if self.userData is None:
            self.userData = SimpleStateElementUserdata()
        self.add_style(EnumShapeStyleFlags.LOCK_CHILDREN | EnumShapeStyleFlags.EMIT_EVENTS | EnumShapeStyleFlags.PROCESS_K_DEL)
        self.titleElement = EditableLabelElement(parent=self, labelType=EnumLabelType.TITLE, text=self.userData.profile.name)
        self.actionsGrid = GridElement(relativePosition=wx.RealPoint(0, 24), parent=self)
        self.entryActionElement = EditableLabelElement(labelType=EnumLabelType.STATE_ENTRY_ACTIONS,
                                                       parent=self.actionsGrid,
                                                       text=self.userData.entryExprText,
                                                       txtFormatter=self.userData.entry_expr_formatter,
                                                       txtValidator=self.userData.validate_entry_expr)
        self.exitActionElement = EditableLabelElement(labelType=EnumLabelType.STATE_EXIT_ACTIONS,
                                                      parent=self.actionsGrid,
                                                      text=self.userData.exitExprText,
                                                      txtFormatter=self.userData.exit_expr_formatter,
                                                      txtValidator=self.userData.validate_exit_expr)
    ...
    def update(self, **kwargs):
        ...
        self.userData.entryExprText = self.entryActionElement.textContent
        self.userData.exitExprText = self.exitActionElement.textContent
