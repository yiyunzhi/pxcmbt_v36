...
class STCElementFactoryItem:
    def __init__(self, name, description, cls, uid, enabled=True, default_kwargs={}, flag=0):
        self.uid = uid
        self.name = name
...
class STCElementFactory:
    ...
    def get_nodes_by_flag(self, flag: int, only_valid=True):
        return {k: v for k, v in self._map.items() if v.flag == flag and (True if not only_valid else v.enabled)}

    def register(self, name, description, cls, uid=None, default_kwargs={}, flag=0):
        if uid is None:
            uid = util_get_uuid_string()
        if uid in self._map:
            raise STCElementFactoryException('uid for registration is already exist.')
        self._map.update({uid: STCElementFactoryItem(name, description, cls, uid, default_kwargs=default_kwargs, flag=flag)})
