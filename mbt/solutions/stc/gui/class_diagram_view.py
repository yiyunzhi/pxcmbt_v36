...
from ..diagram.class_widget_pop_search import NodeComboCtrlPanel
...
from ..diagram.class_stc_diagram_gui_mode import STCDiagramGUIMode
...
class STCDiagramView(wx.Panel):
    ...
    def _create_element(self, identity):
        if self.elementFactory is not None:
            _element = self.elementFactory.get(identity)
            if _element is None:
                return
    ...
    def _reset_tool_mode(self):
        self._currentModeToolId = EnumSTCMenuId.DESIGN_MODE
        self.toolbar.ToggleTool(self._currentModeToolId, aui.AUI_BUTTON_STATE_CHECKED)
        self.toolbar.Refresh()

    def edit_element_prop(self):
        # diagramView has attribute actionsMgr,VariableMgr
        # todo: get function/IOD rendered into gui
        # todo: actionRepository/IODRepository, both de- and serializable, by ZViewContent managed.
        _affect_elem = self.currentShapeEventAffectedElement
        if _affect_elem is None:
            FeedbackDialogs.show_msg_dialog(_('Invalid Operation'), _('affected element is invalid'))
            return
        print('---->edit element prop', _affect_elem)

    def edit_stc_prop(self):
        print('---->edit STC prop')
    ...
    def on_menu(self, evt: wx.CommandEvent):
        ...
        if _id == wx.ID_COPY:
            ...
            self.edit_element_prop()
        elif _id == EnumSTCMenuId.EDIT_PROP:
            self.edit_stc_prop()
            ...
            if _shape is not None and FeedbackDialogs.show_yes_no_dialog(_('Remove Element'),
                                                                         _('Are you sure to remove the selected element.')):
                self.view.scene.remove_shape(_shape)
        elif _id == EnumSTCMenuId.CREATE_TRANSITION:
            if _shape is not None:
                if self.elementFactory is None:
                    return
                _item = self.elementFactory.get_item(IDENTITY_TRANSITION)
                if _item:
                    self.view.guiMode.start_interactive_connection(_item.klass,
                                                                   _shape.get_center(),
                                                                   start_shape=_shape,
                                                                   shape_options=_item.defaultKwargs)
    ...
    def on_graphview_left_down(self, evt: wx.MouseEvent):
        if self._pendedElementCreate is not None:
            ...
            print('todo: conn start with result:', _ret)
            """
